...
$query = "SELECT 
    ...
    (SELECT COUNT(*) FROM tasks WHERE status = 'دەستپێکراوە') AS in_progress_tasks,
...
$in_progress_tasks_count = $row['in_progress_tasks']; // New variable
$devices_count = $row['devices'];

// Add query to count checkups
$checkup_query = "SELECT COUNT(*) AS checkup_count FROM checkup";
$checkup_result = mysqli_query($conn, $checkup_query);
$checkup_row = mysqli_fetch_assoc($checkup_result);
$checkup_count = $checkup_row['checkup_count'];
...
<body class="flex flex-col min-h-screen p-4">
    ...
    <div class="glass flex flex-wrap justify-center gap-4 max-w-7xl w-full mx-auto mb-6 btn-group">
        <a href="../views/tasks.php" class="dashboard-btn px-6 py-3 rounded-pill text-center"><i class="fas fa-thumbtack"></i> ئەركەكانی ڕۆژانە</a>
        <a href="../views/daily_check.php" class="dashboard-btn px-6 py-3 rounded-pill text-center"><i class="fa-solid fa-magnifying-glass-location"></i> پشکنینی ڕۆژانە</a>
        <a href="../views/devices.php" class="dashboard-btn px-6 py-3 rounded-pill text-center"><i class="fas fa-cogs"></i> ئامێرەكان</a>
        <a href="../views/ctrluser/users.php" class="dashboard-btn px-6 py-3 rounded-pill text-center"><i class="fas fa-users"></i> بەكارهێنەران</a>
        <a href="../views/telegram_bot.php" class="dashboard-btn px-6 py-3 rounded-pill text-center"><i class="fas fa-robot"></i> بۆتی تیلیگرام</a>
        <a href="../includes/backup_db.php" class="dashboard-btn px-6 py-3 rounded-pill text-center"><i class="fa-solid fa-cloud-arrow-up"></i> باكئەپ</a>
    </div>
    ...
    <section class="glass max-w-7xl w-full mx-auto mt-10 flex flex-wrap justify-center gap-6 text-center">
        <div class="flex flex-col items-center justify-center space-y-2 w-48 p-4 glass animate__animated animate__fadeInUp">
            ...
            <h3 class="text-lg ">هەموو ئەركەكان</h3>
            <p class="text-3xl font-bold"><?= $all_tasks_count; ?></p>
        </div>

        <div class="flex flex-col items-center justify-center space-y-2 w-48 p-4 glass animate__animated animate__fadeInUp">
            <div class="text-4xl"><i class="fas fa-check-circle"></i></div>
            <h3 class="text-lg "> تەواوبووەکان</h3>
            <p class="text-3xl font-bold"><?= $completed_tasks_count; ?></p>
        </div>

        <div class="flex flex-col items-center justify-center space-y-2 w-48 p-4 glass animate__animated animate__fadeInUp">
            <div class="text-4xl"><i class="fa-solid fa-hourglass-start"></i></div>
            <h3 class="text-lg ">چاوەڕوانی</h3>
            <p class="text-3xl font-bold"><?= $pending_tasks_count; ?></p>
        </div>

        
        <div class="flex flex-col items-center justify-center space-y-2 w-48 p-4 glass animate__animated animate__fadeInUp">
            <div class="text-4xl"><i class="fas fa-hourglass-half"></i></i></div>
            <h3 class="text-lg">دەستپێکراوە</h3>
            <p class="text-3xl font-bold"><?= $in_progress_tasks_count; ?></p>
        </div>

        <div class="flex flex-col items-center justify-center space-y-2 w-48 p-4 glass animate__animated animate__fadeInUp">
            <div class="text-4xl"><i class="fa-solid fa-clipboard-list"></i></div>
            <h3 class="text-lg">پشكنینی ڕۆژانە</h3>
            <p class="text-3xl font-bold"><?= $checkup_count; ?></p>
        </div>
