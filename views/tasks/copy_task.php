...
if (isset($_GET['id'])) {
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        ...
        $image_url = isset($row['image_url']) ? $row['image_url'] : ''; // Define $image_url

        // زیادکردنی کارە نوێیەکە
        $query = "INSERT INTO tasks (task_name, task_number, location, employee, mobile_number, team, status, cost, currency, date, files, image_url) 
                  VALUES ('$task_name', '$task_number', '$location', '$employee', '$mobile_number', '$team', '$status', '$cost', '$currency', '$date', '$files', '$image_url')";
