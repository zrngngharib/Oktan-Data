...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_num_rows($result) === 1) {
        ...
        if (password_verify($password, $user['password'])) { // تایبەتمەندی بە شێوەی تایبەتمەندەکان
            ...
            $_SESSION['role'] = $user['role_id']; // تایبەتمەندانی `role`

            // Log user login
            $user_id = $user['id'];
            $ip_address = $_SERVER['REMOTE_ADDR'];
            $login_time = date('Y-m-d H:i:s');
            $log_query = "INSERT INTO user_activity_log (user_id, action, ip_address, login_time, created_at) 
                          VALUES ($user_id, 'login', '$ip_address', '$login_time', '$login_time')";
